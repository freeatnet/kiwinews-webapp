...
export async function fetchAllStories() {
  ...
  for (let from = 0; true; from += KIWISTAND_MAX_MESSAGES_PER_PAGE) {
    ...
    // eslint-disable-next-line no-console
    console.time(`[${runId}] fetchAllStories page=${from} fetch`);
    ...
    // eslint-disable-next-line no-console
    console.timeEnd(`[${runId}] fetchAllStories page=${from} fetch`);

    // eslint-disable-next-line no-console
    console.time(`[${runId}] fetchAllStories page=${from} parse`);
    const { data } = STORIES_API_RESPONSE_SCHEMA.parse(responseJson);
    // eslint-disable-next-line no-console
    console.timeEnd(`[${runId}] fetchAllStories page=${from} parse`);
